"""
Txl GPT - Work mode engine
------------------------------------
Powers Work mode: a tool-using agent that can list directories, read
files, write files, run shell commands, and (via txlgpt_app.py's added
web_search tool) search the web - inside a sandboxed workspace folder,
using whichever chat backend (Groq or Ollama) txlgpt_app.py currently
has loaded as `agent`. A separate file from TXL Cloud's Code mode engine
(code_agent.py), on purpose - the two apps share no code, and each has
its own workspace folder on disk so they can never collide.

Safety model:
  - Read-only tools (list_directory, read_file, fetch_url, web_search)
    run automatically.
...
Workspace: defaults to a workspace_txlgpt/ folder next to this file, so
the agent can't accidentally touch this app's own source (or TXL Cloud's
workspace/ folder). Point it at a real project instead by setting
TXLGPT_CODE_WORKSPACE to that folder's path.

Multi-account isolation: this app has real user accounts (txlgpt_models.py),
so every tool call is scoped to a per-user subfolder via user_workspace(uid)
...
WORKSPACE_ROOT = Path(os.environ.get("TXLGPT_CODE_WORKSPACE", BASE_DIR / "workspace_txlgpt")).resolve()
WORKSPACE_ROOT.mkdir(parents=True, exist_ok=True)

# Off by default (and MUST stay off on the shared live site) - lets Work
...
# Set TXLGPT_ALLOW_CUSTOM_WORKSPACE=1 in that local deployment's environment only.
ALLOW_CUSTOM_WORKSPACE = os.environ.get("TXLGPT_ALLOW_CUSTOM_WORKSPACE", "0") == "1"
...
SYSTEM_PROMPT_TEMPLATE = """You are Txl GPT in Work mode: an agent with \
tools to inspect and modify files, and run shell commands, inside the \
user's local workspace at {workspace}. You cannot see or touch anything \
outside that folder.
...
def _tool_fetch_url(args: dict) -> str:
    ...
    try:
        resp = requests.get(url, timeout=FETCH_TIMEOUT, headers={"User-Agent": "TxlGPT-WorkMode/1.0"})
