"""
Txl GPT - Gemini vision & image generation
------------------------------------
Two jobs, both via Google's Gemini API: (1) understanding a pasted image
in chat (Groq's models aren't multimodal, so txlgpt_groq.py routes here
instead), and (2) the standalone image-generation feature (the 🖼
composer button). A separate file from TXL Cloud's Gemini connector
(txl_gemini.py), on purpose - the two apps share no code.
...
def stream_reply(
    ...
    """
    Same output shape as txlgpt_groq.stream_reply (yields text chunks),
    but takes an already-assembled system_prompt string directly - the
    caller has already merged in custom_instructions etc. If
    image_data_url is set (a "data:image/...;base64,..." string), it's
    attached to the most recent user turn - Gemini is multimodal, unlike
    the Groq/Ollama models.
    """
    contents = []
    for m in history:
